...
const ProfileCard = (doctorDetails) => {
    ...
    <div className="flex items-center justify-center bg-[#B8DEE2] p-4 md:p-8 h-[250px] md:h-[300px] fixed top-0 left-0 right-0">
      <div className="flex flex-col md:flex-row items-center justify-center container mx-auto">
        ...
        <div className="mt-3 md:mt-0 md:ml-12 text-center md:text-left">
          <h1 className={` ${poppins.className}"text-2xl md:text-[34px] text-black font-medium `}>Dr. {DrDetails?.firstname}</h1>
          <p className="text-[14px] md:text-[18px] text-gray-600 mt-1">{DrDetails?.jobTitle}</p>
          <p className="text-[12px] md:text-[16px] text-gray-500 mt-1 md:mt-2">{DrDetails?.experience} yrs experience</p>
        </div>
      </div>
    </div>
  );
