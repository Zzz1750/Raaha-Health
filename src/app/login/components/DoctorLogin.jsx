...
import { FetchDoctorLogin } from "../../../SERVICE/authService";
import { useRouter } from "next/navigation";
import { useAppDispatch} from "../../../store/hooks";

export default function DoctorLogin({changeLoginMode}){ 
      const [disableButton, setDisableButton] = useState(true);
        ...
        const handleSubmit = async() => {
           try {
             const response = await FetchDoctorLogin(email,password);
    ...
    return(
         <div className="relative flex flex-col justify-center items-center gap-20 ">    
                    ...
                    <button onClick={()=>{changeLoginMode('user')}} className={`border-b-2 rounded-lg   w-40 border-gray-300 `}>User</button>
                    <button onClick={()=>{changeLoginMode('doctor')}} className={`border-b-2 rounded-lg  w-40 border-green-300`}>Doctor</button>
                    </div>
                    <div className="flex flex-col justify-center items-center  border-4 border-gray-300 rounded-lg h-96 w-96 align-middle gap-5 bg-green-100">
